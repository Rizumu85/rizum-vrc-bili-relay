...
const idAllocators = new WeakMap();
function idAllocatorFor(renderer) {
...
export function createRoot(renderer) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    container = reconciler.createContainer(gpuixContainer, ConcurrentRoot, null, false, null, "", console.error, console.error, console.error, null);
    return {
        render: (node) => {
            ...
            reconciler.updateContainer(React.createElement(GpuixContext.Provider, { value: { renderer: batchedRenderer } }, node), activeContainer, null, () => { });
        },
        unmount: cleanup,
    };
}

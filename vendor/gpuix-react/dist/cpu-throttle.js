...
export const MAC_CPU_THROTTLES = ["utility", "background", "maintenance"];
function isMacCpuThrottle(value) {
...
export function readMacCpuThrottle() {
    const raw = (process.env.THROTTLE ?? "").trim().toLowerCase();
    if (!raw)
        return null;
    if (!isMacCpuThrottle(raw)) {
        throw new Error(`THROTTLE=${raw} is invalid. Use utility, background, or maintenance.`);
    }
    return raw;
}
/** Re-exec under `taskpolicy -c`. Call from the process entry, not a vitest worker. */
export function applyMacCpuThrottleFromEnv() {
    const mode = readMacCpuThrottle();
    if (!mode)
        return null;
    if (process.env.GPUIX_CPU_THROTTLE_APPLIED === mode)
        return mode;
    if (process.platform !== "darwin") {
        throw new Error(`THROTTLE=${mode} needs macOS taskpolicy`);
    }
    if (process.argv.some((arg) => arg.includes("vitest/dist/workers"))) {
        throw new Error(`THROTTLE=${mode} must wrap the vitest process. Use examples/vitest.config.ts.`);
    }
...
//# sourceMappingURL=cpu-throttle.js.map
...
foreach ($path in @($archive, $checksum, $report, $mediaReport, $assClockReport, $uiStateReport, $toolchainReport, $notes)) {
...
gh release create $tag $archive $checksum $report $mediaReport $assClockReport $uiStateReport $toolchainReport --repo $env:GITHUB_REPOSITORY --target $env:GITHUB_SHA --draft --title "$tag - UI ownership and persistence" --notes-file $notes

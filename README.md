...
软件以 VRCDN 的使用流程为目标；每个版本的实际验证范围见发布说明。设置页允许填写自定义服务器和播放地址，但其他服务需要自行确认推流协议、播放格式和公网可访问性。
...
- 初次生成、手动暂停和播完暂停时会持续发送生成的提示画面与静音，不会提前消耗视频或主动断开中继；提示画面不是截取视频的最后一帧；
- 跳转、分 P 和弹幕更新只替换内部媒体源，保持同一个 RTMP publisher 与播放地址；
- 播完进入暂停后会保持提示画面一小时；循环、下一 P 和合集下一集会在同一个 publisher 内继续；
